...
use franim::AnimationState;

#[rstest]
fn should_starts_at_frame_zero() {
    let animation: AnimationState = AnimationState::from_frame_duration(3, Duration::from_secs(1));
...
fn should_update_frames_according_to_elapsed_time(
    ...
    let mut animation = AnimationState::from_frame_duration(3, Duration::from_secs(1));
...
fn zero_frame_duration_should_panics() {
    let _ = AnimationState::from_frame_duration(3, Duration::ZERO);
}
